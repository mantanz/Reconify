#!/usr/bin/env python3
"""
Test script to verify panel save audit logging functionality
"""
...
TEST_PANEL = {
    "name": f"test_panel_save_{timestamp}",
    "key_mapping": {
        "hr_data": {
            "email": "email"
        }
...
def test_panel_save_audit():
    """Test panel save with audit logging"""
    print("🧪 Testing Panel Save Audit Logging")
    print("=" * 50)
    
    try:
        # Test panel save
        print("1. Saving test panel...")
        response = requests.post(
            f"{BASE_URL}/panels/save",
            json=TEST_PANEL,
        ...
        if response.status_code == 200:
            ...
            print(f"   ❌ Failed to save panel: {response.status_code} - {response.text}")
            return False
        ...
        print("\n🎉 Panel Save Audit Test Complete!")
        return True
